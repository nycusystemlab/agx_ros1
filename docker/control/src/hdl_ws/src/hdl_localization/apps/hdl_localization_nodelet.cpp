...
private:
  ...
  void imu_callback(const sensor_msgs::ImuConstPtr& imu_msg) {
    ...
    //ROS_INFO("imu_data.size 1 time");
    std::cout<<"imu_data.size 1  ()  = "<< imu_data.size() <<std::endl;
  }
...
//點雲下採樣
    ...
    if(!use_imu) {  //不使用imu
      ...
      for(imu_iter; imu_iter != imu_data.end(); imu_iter++) {  //imu數據迭代
        if(stamp < (*imu_iter)->header.stamp) {
          break;
        }
        const auto& acc = (*imu_iter)->linear_acceleration;  //讀取數據
        const auto& gyro = (*imu_iter)->angular_velocity;
        double acc_sign = invert_acc ? -1.0 : 1.0;           //是否轉置
      ...
      //ROS_INFO("2 time");
      std::cout<<"imu_data.size() 2  = "<<imu_data.size()<<std::endl;
      
    }

    // odometry-based prediction
    ros::Time last_correction_time = pose_estimator->last_correction_time();  //將最後一次校正的時間儲存
    if(private_nh.param<bool>("enable_robot_odometry_prediction", false) && !last_correction_time.isZero()) { //如果最後一次校正的時間不為零，執行以下程式碼
      geometry_msgs::TransformStamped odom_delta;  //存儲查找到的轉換結果
      ...
      if(odom_delta.header.stamp.isZero()) {  //檢查 odom_delta 的時間戳是否為零
