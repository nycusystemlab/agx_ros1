...
private:
    ...
    // -------- 線性加速度卡爾曼濾波變數 --------
    bool kf_initialized_x_;      // 初始化 X 軸線性加速度濾波
    ...
    // -------- 角速度卡爾曼濾波變數 --------
    // 是否初始化 Roll 軸角速度濾波
    bool kf_initialized_roll_, kf_initialized_pitch_, kf_initialized_yaw_;   // 是否初始化 3 軸角速度濾波
    float x_kf_roll_rate, x_kf_pitch_rate, x_kf_yaw_rate;           // 3 軸濾波後的角速度估計值 (rad/s)
    float P_kf_roll_, P_kf_pitch_, P_kf_yaw_;           // 3 軸濾波的協方差
    ...
    // -------- 为 motion_a_real_ 新增 KF 变量 --------
    bool   kf_initialized_a_;    // 是否初始化
    float  x_kf_motion_a_;       // 滤波后的加速度估计
    float  P_kf_motion_a_;       // 估计误差协方差
    const float Q_kf_motion_a_;  // 过程噪声方差
    const float R_kf_motion_a_;  // 测量噪声方差
...
TeleopKeyboard::TeleopKeyboard() : 
    ...
    x_kf_acc_x_(0.0f), P_kf_x_(1.0f), 
    x_kf_acc_y_(0.0f), P_kf_y_(1.0f),
    x_kf_acc_z_(0.0f), P_kf_z_(1.0f),

    kf_initialized_roll_(false), x_kf_roll_rate(0.0f), P_kf_roll_(1.0f),
    kf_initialized_pitch_(false), x_kf_pitch_rate(0.0f), P_kf_pitch_(1.0f),
    kf_initialized_yaw_(false), x_kf_yaw_rate(0.0f), P_kf_yaw_(1.0f),
    kf_initialized_omega_fused_(false), x_kf_omega_fused_(0.0f), P_kf_omega_fused_(1.0f),

    Q_kf_linear_(0.01f),  // 線性加速度過程雜訊方差，較小值表示信任模型動態
    R_kf_linear_(0.01f),    // 線性加速度測量雜訊方差，根據 IMU 雜訊特性設定
    Q_kf_angular_(0.05f), // 角速度過程雜訊方差，稍大以適應角速度的快速變化
    R_kf_angular_(0.8f),   // 角速度測量雜訊方差，角速度通常比線性加速度雜訊更大
    
     // 初始化 motion_a_real_ KF 参数
    kf_initialized_a_(false),
    x_kf_motion_a_(0.0f),
    P_kf_motion_a_(1.0f),
    Q_kf_motion_a_(0.1f),   // 过程噪声：可根据实验调
    R_kf_motion_a_(0.75f),     // 测量噪声：根据 motion_a_real_ 的噪声特性调
    
    last_imu_time_(0),
    last_motion_time_(0)
{
    private_nh_.param<std::string>("save_filename", filename_, "/home/systemlabagx/disk/0701_test.csv");
...
void TeleopKeyboard::imuCallback(const sensor_msgs::ImuConstPtr &msg) {
    ...
    // x_kf_yaw_rate = 0.5 * x_kf_yaw_rate + 0.5 * pre_x_kf_yaw_rate;
    // 發布濾波後的 IMU 數據
...
void TeleopKeyboard::motionArrayCallback(const std_msgs::Float32MultiArrayConstPtr &msg) {
    if (msg->data.size() == 15) {
        ...
        // if (!kf_initialized_a_) {
        //     x_kf_motion_a_   = static_cast<float>(motion_a_real_);
        //     P_kf_motion_a_   = 1.0f;
        //     kf_initialized_a_ = true;
        // } else {
        //     // Predict
        //     float x_pred = x_kf_motion_a_;
        //     float P_pred = P_kf_motion_a_ + Q_kf_motion_a_;

        //     // Update
        //     float z = static_cast<float>(motion_a_real_);
        //     float K = P_pred / (P_pred + R_kf_motion_a_);
        //     x_kf_motion_a_   = x_pred + K * (z - x_pred);
        //     P_kf_motion_a_   = (1.0f - K) * P_pred;
        // }
        ...
        //82.5
        float omega_meas_cmd_fused = 0.1 * fabs(omega_cmd) + 0.9 * fabs(x_kf_yaw_rate);
        // float omega_meas_cmd_fused = fabs(x_kf_yaw_rate);
        // float abs_liner_velocity = 0;

        // if(pose_yaw <= -0.02){
        //     abs_liner_velocity = 0.3 * fabs(velocity_cmd) + 0.7 * fabs(motion_v_real_);
        // }else{
        //     abs_liner_velocity = fabs(motion_v_real_);
        // }

        // float abs_liner_velocity = 0.2 * fabs(velocity_cmd) + 0.8 * fabs(motion_v_real_);
        float abs_liner_velocity = fabs(motion_v_real_);
        float c0_set = 0;
        if ( cmd_.data == 6 || cmd_.data == 7 ){ // differential steering
            c0_set = c1_3rd;
            if(abs_liner_velocity >= velocity_cmd){
                abs_liner_velocity = velocity_cmd;
            }
        }else if(cmd_.data == 3 || cmd_.data == 4){    // 原地旋轉
            c0_set = c1_2nd;
            if(abs_liner_velocity >= velocity_cmd){
                abs_liner_velocity = velocity_cmd;
            }
        }else{                                   // gerneral condition
            c0_set = cofficient[0];
        }
        power_regression = c0_set +
                            cofficient[1] * abs_liner_velocity +
                            cofficient[2] * std::max(motion_a_real_, 0.0) * abs_liner_velocity +
                            cofficient[3] * std::min(motion_a_real_, 0.0) * abs_liner_velocity +
                            cofficient[4] * omega_meas_cmd_fused +
                            cofficient[5] * std::max(angular_acceleration_yaw_, 0.0f) * omega_meas_cmd_fused +
                            cofficient[6] * std::min(angular_acceleration_yaw_, 0.0f) * omega_meas_cmd_fused +
                            cofficient[7] * std::max(std::sin(-pose_pitch), 0.0) * abs_liner_velocity +
                            cofficient[8] * std::min(std::sin(-pose_pitch), 0.0) * abs_liner_velocity;
