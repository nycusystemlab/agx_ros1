    ...
    last_motion_time_(0)
{
    ...
    if (!ofs.is_open()) {
        ROS_WARN("Unable to open file %s", filename_.c_str());
    } else {
...
void TeleopKeyboard::motionArrayCallback(const std_msgs::Float32MultiArrayConstPtr &msg) {
    if (msg->data.size() == 15) {
        ...
        //82.5
        // float omega_meas_cmd_fused = 0.5 * fabs(omega_cmd) + 0.5 * fabs(x_kf_yaw_rate);
        ...
        if(fabs(velocity_cmd) > 0 && fabs(omega_cmd) > 0){        // differential steering
            c0_set = c1_3rd;
            if(abs_liner_velocity >= velocity_cmd){
                abs_liner_velocity = velocity_cmd;
            }
        }else if(fabs(velocity_cmd) == 0 && fabs(omega_cmd) > 0){ // 原地旋轉
            c0_set = c1_2nd;
        }else{
            c0_set = cofficient[0];
        }
        ...
        power_regression = c0_set +
                            ...
                            cofficient[7] * std::max(std::sin(pose_pitch * -1), 0.0) * abs_liner_velocity +
                            cofficient[8] * std::min(std::sin(pose_pitch * -1), 0.0) * abs_liner_velocity;
