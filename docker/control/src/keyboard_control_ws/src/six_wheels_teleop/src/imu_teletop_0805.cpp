...
private:
    ...
    float power_regression_lidar = 0, power_regression_imu = 0;
    // -------- 角加速度和時間戳 --------
...
TeleopKeyboard::TeleopKeyboard() : 
    ...
    last_motion_time_(0)
{
    private_nh_.param<std::string>("save_filename", filename_, "/home/systemlabagx/disk/0907_test.csv");
...
void TeleopKeyboard::motionArrayCallback(const std_msgs::Float32MultiArrayConstPtr &msg) {
    if (msg->data.size() == 15) {
        ...
        float cofficient[9] = {28.92f, 300.96f, 92.8f, -3.5f, 298.6f, 11.13f, -8.16f, 1037.04f, 956.38f};
        ...
        power_regression_imu = c0_set +
                            ...
                            cofficient[8] * std::min(std::sin(imu_roll + 1.62), 0.0) * abs_liner_velocity;
